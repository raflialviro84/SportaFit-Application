...
import { toast } from "react-toastify"; // Import toast jika tersedia, jika tidak ada bisa diabaikan

export default function VoucherPage() {
  const navigate = useNavigate();
  const { vouchers, userVouchers, loading, error, addVoucherToUser, refreshVouchers } = useVoucher();
  ...
  const handleAddVoucher = async (e) => {
    ...
    try {
      ...
      // Tambahkan toast notification jika tersedia
  ...
  // Fungsi untuk mengklaim voucher langsung dari kartu
  const handleClaimVoucher = async (code) => {
    ...
    try {
      const result = await addVoucherToUser(code);
      ...
      setAddVoucherError(err.message || "Gagal klaim voucher");
  ...
  // Fungsi untuk menggunakan voucher dalam pemesanan
  const handleUseVoucher = (voucher) => {
    // Navigasi ke halaman daftar arena dengan parameter voucher
    navigate(`/arena?voucher=${voucher.id}`);
  };

  // Efek untuk menghapus pesan sukses/error setelah 5 detik
  useEffect(() => {
    if (addVoucherSuccess || addVoucherError) {
  ...
  }, [addVoucherSuccess, addVoucherError]);
            ...
            return (
              ...
              >
                <div className="h-40 w-full relative">
                  <img
                    src={v.imageUrl || `/Voucher${v.id % 4 + 1}.png`}
                    alt={v.title}
                ...
                <div className="p-4 space-y-2">
                  ...
                  {isClaimed ? (
                    isUsed ? (
                      <button
                        className="mt-2 w-full px-4 py-2 bg-gray-300 text-gray-600 rounded-full text-sm font-medium cursor-not-allowed"
                        disabled
                      >
                        Sudah Digunakan
                      </button>
                      ...
                      <button
                        ...
                        className="mt-2 w-full px-4 py-2 bg-green-500 text-white rounded-full text-sm font-medium hover:bg-green-600 transition-colors"
                      >
                        Gunakan Voucher
                      </button>
                    )
                    ...
                    <button
                      ...
                      className="mt-2 w-full px-4 py-2 bg-sporta-blue text-white rounded-full text-sm font-medium hover:bg-blue-700 transition-colors"
                      disabled={addingVoucher}
                    >
                      {addingVoucher ? "Mengklaim..." : "Klaim Voucher"}
                    </button>
                  )}
                </div>
              </div>
