...
export default function DetailTransaction() {
      ...
      try {
        ...
        if (data) {
          ...
          const enhancedData = {
            ...
            protectionFee: data.protectionFee ?? data.protection_fee ?? 0,
            // Handle voucher information
            voucherInfo: data.voucherInfo || data.voucher || null,
            voucherDiscount: data.discount ?? data.voucherDiscount ?? 0,
            promo: data.discount ?? data.voucherDiscount ?? 0,
            ...
            payment: {
              ...
              total: data.finalTotalAmount ?? data.final_total_amount ?? ((data.totalPrice ?? data.total_price ?? 0) + (data.serviceFee ?? data.service_fee ?? 5000) - (data.discount ?? data.voucherDiscount ?? 0))
            }
          };
  ...
  const promo = tx.discount ?? tx.promo ?? tx.voucherDiscount ?? 0;
  const totalPayment = tx.finalTotalAmount ?? tx.final_total_amount ?? (price + serviceFee + protectionFee - promo);

  // Format voucher information for display
  const hasVoucher = promo > 0 || (tx.voucherInfo && (typeof tx.voucherInfo === 'object' || typeof tx.voucherInfo === 'string'));
  const voucherName = tx.voucherInfo ? (
    typeof tx.voucherInfo === 'object' 
      ? (tx.voucherInfo.title || tx.voucherInfo.name || 'Voucher') 
      : (typeof tx.voucherInfo === 'string' ? tx.voucherInfo : 'Voucher')
  ) : 'Voucher';
      ...
      <main className="pt-20 pb-24 max-w-[434px] mx-auto px-4 space-y-6">
        ...
        <section className="bg-gray-100 rounded-2xl px-4 py-4 space-y-3">
          ...
          <div className="space-y-2 text-sm text-gray-700">
            ...
            {/* Voucher/Promo Section - Only show if there's a voucher applied */}
            {hasVoucher && (
              <div className="flex justify-between items-center">
                <div className="flex items-center">
                  <IoTicketOutline size={16} className="text-green-600 mr-1" />
