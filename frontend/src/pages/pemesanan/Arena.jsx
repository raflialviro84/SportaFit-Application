...
// Array gambar hero untuk 5 arena teratas
const heroImages = [
  "/foto_lapangan.png",
  "/foto_lapangan1.jpg",
  "/foto_lapangan2.jpg",
  "/foto_lapangan3.jpg",
  "/lapangan-badminton.png"
];

// Fungsi untuk mendapatkan gambar hero berdasarkan index arena
const getHeroImage = (index, arena) => {
  // Untuk 5 arena teratas, gunakan gambar hero yang berbeda
  if (index < 5) {
    return heroImages[index];
  }
  // Untuk arena lainnya, gunakan gambar default atau dari database
  return arena.image_url || arena.img || '/foto_lapangan.png';
};
...
const mockArenas = [
  ...
  },
  {
    ...
    address: "Jl. Raya Gubeng No.45",
    city: "Surabaya",
    image_url: "/lapangan-badminton.png",
    rating: 4.8,
    reviews_count: 201,
  }
];
...
function Arena() {
  ...
  const voucherId = searchParams.get('voucher'); // Extract voucher ID from URL
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] px-4 pt-6 pb-24">
      <div className="max-w-[434px] mx-auto space-y-5 font-jakarta">
        ...
        {/* Overlay Cards */}
        {!loading && !error && arenas.map((arena, index) => (
          <div
            key={arena.id}
            className="relative h-[250px] rounded-2xl overflow-hidden mb-4 cursor-pointer"
            onClick={() => navigate(`/arena/${arena.id}${voucherId ? `?voucher=${voucherId}` : ''}`)}
          >
            {/* Background Image */}
            <div
              className="absolute inset-0 bg-center bg-cover"
              style={{ backgroundImage: `url(${getHeroImage(index, arena)})` }}
            />
